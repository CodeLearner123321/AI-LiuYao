...
public class ConstantUtil {
    ...
    public final static String IMAGE_PROCESSING_PROMPT_WORDS2 = "你有一个任务：提取图片中的有效信息,并按照Json格式返回(请注意只用返回字符串信息就可以了，除此之外不要返回任何其他的特殊字符比如)。同时你知道你返回的数据将会被转换成一个Java对象，所以你必须严格返回数据。 现在为了方便处理，你可以把你的提取任务分为三个方面。 1、提取问题与背景 2、提取卦象（本卦与变卦，如果没有变卦，则变卦和本卦一样。） 3、提取时间（时间分为 年月日时，年月日时又分为数字时间和干支,如果出现时间没有给出或者没有给全等不明确的情况，千万不要自己造一个数据，没有就是null） 现在请你提取，提取的例子如下： { \"description\": { \"question\": \"预测企业发展前景\", \"background\": \"2018年，钢厂老板李某前来求测\" }, \"gua\": { \"zhuGua\": \"雷天大壮\", \"bianGua\": \"雷天大壮\" }, \"time\": { \"year\": { \"ganzhi\": null, \"time\": 2018 }, \"month\": { \"ganzhi\": { \"tiangan\": \"戌\", \"dizhi\": \"午\" }, \"time\": null }, \"day\": { \"ganzhi\": { \"tiangan\": \"丙\", \"dizhi\": \"戌\" }, \"time\": null }, \"hour\": { \"ganzhi\": null, \"time\": null } } }?";
    public final static String IMAGE_SYSTEM_PROMPT = "你是一个资深的六爻专家，同时也是一名高级后端开发程序员";
}
